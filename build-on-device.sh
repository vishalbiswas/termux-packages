...
termux_step_handle_arguments() {
	...
	_show_usage () {
	    echo "Usage: ./build-on-device.sh [-d] [-D] PACKAGE"
	...
	while getopts :hdDfs option; do
		...
		?) termux_error_exit "./build-on-device.sh: illegal option -$OPTARG";;
		esac
	done
	shift $((OPTIND-1))
...
termux_step_setup_variables() {
	...
	: "${TERMUX_ARCH=`dpkg --print-architecture`}" # cannot use uname because armv7l vs arm
	: "${ORIG_PREFIX:="$PREFIX"}"
	: "${TERMUX_PREFIX:="$ORIG_PREFIX/local"}"
	: "${TERMUX_ANDROID_HOME:="/data/data/com.termux/files/home"}"
	: "${TERMUX_DEBUG:=""}"
	: "${TERMUX_API_LEVEL:="21"}"
	: "${TERMUX_ANDROID_BUILD_TOOLS_VERSION:="25.0.1"}"
	: "${TMPDIR:="$ORIG_PREFIX/tmp"}"
	: "${CONFIG_SHELL:="$ORIG_PREFIX/bin/sh"}"
	...
	TERMUX_DX=dx
	TERMUX_JACK=$TERMUX_PREFIX/share/dex/jack.jar

	TERMUX_COMMON_CACHEDIR="$TERMUX_TOPDIR/_cache"
	TERMUX_DEBDIR="$TERMUX_SCRIPTDIR/debs"
	TERMUX_ELF_CLEANER=$TERMUX_COMMON_CACHEDIR/termux-elf-cleaner

	TERMUX_SYSTEM_LIBDIR=/system/lib
	test $TERMUX_ARCH_BITS == 64 && TERMUX_SYSTEM_LIBDIR+=64
	...
	export PKG_CONFIG_PATH=$PKG_CONFIG_LIBDIR:$ORIG_PREFIX/lib/pkgconfig
...
termux_step_start_build() {
	...
	if [ -n "$TERMUX_PKG_HOSTBUILD" ]; then
		echo "WARNING: hostbuild is not implementated correctly for on-device builds"
	fi

	if [ "$TERMUX_PKG_CLANG" == "no" ]; then
		echo "WARNING: gcc is not available on termux, build may fail"
	fi
	...
	if [ "$TERMUX_ELF_CLEANER_SRC" -nt "$TERMUX_ELF_CLEANER" ]; then
		clang++ -std=c++11 -Wall -Wextra -pedantic -Os "$TERMUX_ELF_CLEANER_SRC" -o "$TERMUX_ELF_CLEANER"
	fi
	...
	test ! -f $TERMUX_PREFIX/libz.so && cp $TERMUX_SYSTEM_LIBDIR/libz.so $TERMUX_PREFIX/libz.so
...
termux_step_setup_toolchain() {
	...
	export ac_cv_func_getpwuid=no
		...
		if [ "$TERMUX_ARCH" = "aarch64" ]; then
			...
			#FIXME: set LD to gold when available in repo
			true
		fi

		#FIXME: this solution doesn't work probably
	if [ ! -f $TERMUX_PREFIX/lib/libstdc++.so ]; then
		...
		_STL_LIBFILE=$ORIG_PREFIX/lib/libgnustl_shared.so
		cp "$_STL_LIBFILE" .
...
termux_step_patch_package() {
	...
	do
		chmod u+w $file
		cp "$TERMUX_SCRIPTDIR/scripts/config.sub" $file
	done < <(find . -name config.sub -print0)
	unset file

	while IFS= read -r -d '' file
	do
		chmod u+w $file
		cp "$TERMUX_SCRIPTDIR/scripts/config.guess" $file
	done < <(find . -name config.guess -print0)
	unset file

	# Fix shebang paths:
	while IFS= read -r -d '' file
	do
		head -c 100 "$file" | grep -E "^#\!.*\\/bin\\/.*" | grep -q -E -v "^#\! ?\\/system" && sed --follow-symlinks -i -E "1 s@^#\!(.*)/bin/(.*)@#\!$ORIG_PREFIX/bin/\2@" "$file"
	done < <(find -L . -type f -executable -print0)
}
...
termux_step_configure_autotools () {
	...
	env $AVOID_GNULIB sh "$TERMUX_PKG_SRCDIR/configure" \
...
termux_step_configure_cmake () {
	...
	cmake -G 'Unix Makefiles' "$TERMUX_PKG_SRCDIR" \
		...
		-DCMAKE_LINKER="$ORIG_PREFIX/bin/$LD $LDFLAGS" \
